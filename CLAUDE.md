...
- **Status**: 2.6.13 — compiles cleanly under cycc 6.5.6. Library source lives in `src/` (smoke `main.cyr` + 34 math modules); `lib/` is vendored stdlib + deps only. CLI smoke binary builds; **full 34-module distlib bundle** (~562 KB / 17,021 lines, well under cycc's 1 MB input_buf) ships at `dist/hisab.cyr` and is consumer-tested end-to-end on 6.5.6. Library validated via tests (**1063/1063** across 4 suites) plus `scripts/check-constants.sh` (110/110). **2.6.12 + 2.6.13 are the audit-repair pair** and close all of P0 from the 2026-08-03 sweep (`docs/audit/2026-08-03.md`, 70 verified findings, 2 critical). 2.6.12: seven hand-encoded IEEE-754 constant tables did not encode their documented values — 47 constants re-derived, incl. DOPRI45 (23 of 30 wrong; Σb was 0.636, so it was **not a consistent integrator at any order**), BDF-4, Yoshida-4, Gauss-Legendre-5 — plus `num_is_prime`'s i64 overflow and `solve_bicgstab`'s no-op tolerance. 2.6.13: `svd_golub_kahan` returned U transposed, `eigen_qr` never converged for n ≥ 3, the SU(2)/SE(3) half-vs-full-angle split, `einsum` mis-computing its own documented examples, and three interval enclosure-soundness violations. **All defects were in modules with zero or near-zero test coverage** — 4 of the 8 never-tested modules are now in the suites (`einsum`, `lie_ext`, `mat3`, `linalg_precision`); `calc_ext`, `noise_simplex`, `num_ext`, `symbolic_ext` remain. `cyrius coverage` **54%** against the 80% target. The prior 6.4.66 bump (2.6.9) renamed `modules.tcyr`'s interval result vars `iv_add`/`iv_sub`/`iv_mul` → `iv_sum`/`iv_diff`/`iv_prod` because the originals collide with reserved cycc SIMD intrinsic names (unusable as variables — `expected identifier, got unknown`, still live on 6.5.6); see `docs/development/issues/2026-07-17-cyrius-interval-ident-lex.md`.
...
dist/hisab.cyr       — full 34-module distlib bundle (~560 KB / 17,021 lines),
