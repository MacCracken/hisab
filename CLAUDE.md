...
- **Status**: 2.6.15 — compiles cleanly under cycc 6.5.6. Library source lives in `src/` (smoke `main.cyr` + 34 math modules); `lib/` is vendored stdlib + deps only. CLI smoke binary builds; **full 34-module distlib bundle** (~578 KB / 17,286 lines, well under cycc's 1 MB input_buf) ships at `dist/hisab.cyr` and is consumer-tested end-to-end on 6.5.6. Library validated via tests (**1127/1127** across 4 suites), 28 benchmarks, and `scripts/check-constants.sh` (110/110). **2.6.12–2.6.15 are the audit-repair arc** and fully discharge the 2026-08-03 sweep (`docs/audit/2026-08-03.md`, 70 verified findings, 2 critical). 2.6.12: seven hand-encoded IEEE-754 constant tables did not encode their documented values — 47 constants re-derived, incl. DOPRI45 (23 of 30 wrong; Σb was 0.636, so it was **not a consistent integrator at any order**), BDF-4, Yoshida-4, Gauss-Legendre-5 — plus `num_is_prime`'s i64 overflow and `solve_bicgstab`'s no-op tolerance; added the constant harness as a CI gate. 2.6.13: `svd_golub_kahan` returned U transposed, `eigen_qr` never converged for n ≥ 3, the SU(2)/SE(3) half-vs-full-angle split, `einsum` mis-computing its own documented examples, three interval enclosure-soundness violations. 2.6.14: the memory-safety tier (capped-constructor null derefs, solver dimension caps + `HSB_ERR_ALLOC`, B-spline negative indexing, adaptive-Simpson work bound, `kdtree_build` SIGSEGV at 60k coincident points, Armijo NaN, complex 1e-6-vs-1e-12 tolerance, `FLOAT_RENDER_BUF`). 2.6.15: both O(n²) hot paths (halfedge twin pairing 190 ms → 1.2 ms; convex-hull pre-sort 22 ms → 2.1 ms, before/after in `bench-history.csv`) plus six doc drifts. **Every defect was in a module with zero or near-zero coverage** — all 34 modules are now included by a suite (eight were not even compiled by `cyrius test` before). `cyrius coverage` **59%** against the 80% target. The 6.4.66 bump (2.6.9) renamed `modules.tcyr`'s interval result vars `iv_add`/`iv_sub`/`iv_mul` → `iv_sum`/`iv_diff`/`iv_prod` because the originals collide with reserved cycc SIMD intrinsic names (unusable as variables — `expected identifier, got unknown`, still live on 6.5.6); see `docs/development/issues/2026-07-17-cyrius-interval-ident-lex.md`.
...
dist/hisab.cyr       — full 34-module distlib bundle (~578 KB / 17,286 lines),
