//! ganit-core — Core math types: vectors, matrices, quaternions, transforms.
//!
...
/// Errors from ganit-core operations.
#[derive(Error, Debug)]
pub enum GanitError {
...
mod tests {
    ...
    fn projection_orthographic_identity_like() {
        ...
        // Should map (-1..1) to (-1..1) in all axes
    ...
    fn projection_perspective_basic() {
        ...
        // Near plane center should map to z = -1 in clip space
    ...
    #[test]
    fn ganit_error_display() {
        let e = GanitError::DivisionByZero;
        assert_eq!(e.to_string(), "division by zero");
        let e = GanitError::SingularMatrix;
        assert_eq!(e.to_string(), "singular matrix — cannot invert");
    }

    #[test]
    fn transform3d_combined() {
        // Translate + scale: point (1,1,1) scaled by 2 then translated by (10,0,0)
