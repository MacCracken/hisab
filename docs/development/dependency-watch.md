...
**Status:** Pinned to **6.4.69** via `cyrius.cyml [package].cyrius` (legacy `.cyrius-toolchain` removed; CI/release grep the manifest directly).
...
**Status:** `sakshi` **2.4.6** via git, modules path `dist/sakshi.cyr` (bumped 2.4.2 → 2.4.6 in v2.6.9; commit-pinned in `cyrius.lock`). 2.4.6 pins cyrius 6.4.49 (≤ our 6.4.69); its shipped surface links only `fnptr` + `atomic` and is unchanged, so behaviour is identical.
