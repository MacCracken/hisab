...
- [x] ✅ **DONE in 2.22.1 — and the decision this row asked for was answered by REFUTING its own
      premise.** It asked whether one global scalar can normalise a `blockdiag(normal, subnormal)`
      matrix. **It can**: f64's normal range spans 2045 binades and these fixtures need ~1076. The
      defect was WHICH scalar — `pow2_floor(max|A|)` normalises the large end only — so the fix is a
      two-ended choice, not per-block scaling and not a refusal. ⛔ And the balance was only HALF the
      cause: `vtv` underflows at the bottom of the NORMAL range, so normalising the reflector was
      required too, and balance alone converts 30 LOUD rows into SILENT wrong answers.
      CORRECT 23 -> 151 of 156, LOUD 62 -> 0, zero regressions.
