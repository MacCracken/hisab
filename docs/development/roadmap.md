...
Suite **4014** across five harnesses (hisab 512, foundation 413, modules 2072, edge_cases 233,
...
| ~~**2.22.0 — the Householder gate**~~ | ✅ **SHIPPED, AND IT WAS NEVER A SUBNORMAL DEFECT.** This row read "subnormal SVD" for three releases. `_lp_bidiagonalize` gated its Householder reflectors on an ABSOLUTE 1e-12 applied to a LENGTH, and when the gate fired the reflector was skipped while the extraction still read only the bidiagonal band — so the dropped entries were silently discarded and the routine returned the factorisation of a DIFFERENT matrix with rc = HSB_ERR_NONE. ⭐ **The boundary is a block ratio of 2^-40, a condition number of ~1e12 — 982 binades above where this row placed it.** Measured oracle-free (prod(S) must equal |det A|): 25 of 60 normal-range ratios silently wrong -> 0, smallest singular value 9.89x too large -> exact. ⛔ **The symmetric twin has the same gate**, so `eigen_qr` returned the spectrum of a different matrix and **the smallest eigenvalue came back with the WRONG SIGN** (+0.438c for -1c); the values returned are exactly the spectrum of the matrix you get by dropping W[3,1], so the mechanism is pinned by the values. ⛔ **A second, separable defect**: `_lp_bidiag_qr`'s `p_lo` search froze the active block at 2x2 on the same absolute test — and **a 2x2 fixture cannot expose it**, which is why the whole 2.20.0 acceptance family missed it. ⛔ **Both entry points returned SUCCESS with a NaN payload** for +Inf/-Inf/NaN input. ⛔ **THE REPAIR THIS ROW PROPOSED WAS AT THE WRONG PIPELINE STAGE and the 2.19.0 refutation that blocked it for three releases is TRUE** — scaling the active QR block converts 84 loud rows into gross silent wrong answers and cannot recover a reflector bidiagonalisation never applied. ⚠ **Seventh consecutive release where the row was wrong about scope**: the grep found 5 sites in 3 functions under 3 public entry points and this row named none of them. Suites **3991 -> 4014**, every new assertion verified to fail on the 2.21.0 tree. | 2.21.0 |
...
- [x] ✅ **MEASURED, THE FALSE CLAIM RETRACTED, THE FIXTURE FAMILY WIDENED — repair filed, not
      ...
      **9.78x**, one of them a smallest singular value returned as exactly ZERO where the truth is 4
      ...
      ⛔ ORIGINAL ROW FOLLOWS.
- [x] ✅ **DONE in 2.22.0 — BUT NOT AS THIS ROW PROPOSED, AND THE ROW'S OWN REPAIR IS REFUTED.**
      The defect was never subnormal: `_lp_bidiagonalize` gated its Householder reflectors on an
      ABSOLUTE 1e-12 applied to a LENGTH, so the boundary is a BLOCK RATIO of 2^-40 — condition
      ~1e12, **982 binades above where this row placed it** — and 97.9% of the silent wrong answers
      are at NORMAL block ratios. Measured oracle-free: 25 of 60 normal-range ratios silently wrong
      -> 0. ⛔ **Scaling the active block inside `_lp_bidiag_qr`, which is what this row proposed,
      converts 84 LOUD rows into gross SILENT wrong answers and cannot recover a reflector that
      bidiagonalisation never applied** — the 2.19.0 refutation that blocked this for three releases
      is TRUE, and what was missing was the measurement saying which side was right.
      ⚠ **A RESIDUE IS OPEN AND SCHEDULED BELOW**, not closed here: the deep-subnormal 2x2
      fabricated zero (the `_sq_ok == 94` pin in tests/hisab.tcyr) is UNCHANGED by this release —
      it is attributed to the entry-point balancing divide, a different site.
      [measured: 60 normal + 52 subnormal block ratios on both trees, 2026-09-11]
      ⛔ ORIGINAL ROW FOLLOWS.
      **[subnormal SVD: scale the active block]** ⚠ **The remaining 28 subnormal block ratios are the GRID, not a defect —
...
- [ ] **[2.22.0, residue — measured and scoped, not folded in]** **The deep-subnormal 2x2 fabricated
      zero survives.** 2.22.0 repaired the Householder gate and the `p_lo` active-block search, and the
      2.20.0 acceptance pin `_sq_ok == 94` is **unchanged** by both — verified, not assumed. The
      remaining cause is attributed to `svd_golub_kahan`'s entry-point balancing DIVIDE
      (linalg_precision.cyr:986), which is lossy for a subnormal sub-block: it divides by
      `pow2_floor(max|A|)`, set by the LARGE block, so the small block loses mantissa bits before the
      decomposition starts. ⭐ **A single global scalar cannot fix this in principle** and that is the
      thing to establish first with numbers: compute the dynamic range a block-diagonal
      [normal, subnormal] matrix needs and compare it against f64's. If it exceeds it, the answer is
      per-block scaling or an honest refusal — not a better global scalar.
      ⚠ Do NOT reach for "scale the active QR block": 2.22.0 measured that converting 84 loud rows to
      silent wrong answers is exactly what it does.
