...
| mat_new (stdlib) | Integer overflow in `16 + rows*cols*8` | **Upstream bug** — still unguarded in pinned 6.3.11 (re-verified 2026-06-30: `ganita.cyr` is byte-identical to the 6.2.11 vendor, so `ganita_mat_new` is unchanged — still no bounds check; 2026-06-15: moved from `matrix.cyr` to `ganita_mat_new`, body unchanged). hisab provides **`mat_new_guarded`** (2.5.3) as the safe constructor for untrusted dims (cap 16M elems); internal `mat_new` calls use dims from already-allocated matrices (mitigated); raw-dim `cmat_new` is guarded |
