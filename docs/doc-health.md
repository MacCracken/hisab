...
| `2026-08-06-ghost-incircle-g1-g3-assume-ccw.md` | 2026-08-06 | 🟡 **OPEN, filed not fixed** — fallout from the above. `_col_dl_ic_g1`/`_col_dl_ic_g3` drop the winding factor entirely (**462,846 of 463,086** and **243 of 243** wrong on a CW triple), so `_col_dl_incircle` is CCW-only and the 2.9.1 fix does not change that. Carries a **second, orientation-independent** defect: `g1`'s M¹ tie-break answers "outside" for a point strictly between `a` and `b` when the edge is parallel to `u_k` (228 of 463,086 on correctly CCW input). Evidence is a Python transcription, not a first-party assertion — the file says so. |
