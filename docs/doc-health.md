...
> **Last refresh**: 2026-09-14 (post-3.2.0) — **documentation sweep.** 18 source doc comments and
...
| File | Last touched | Status | Action |
|---|---|---|---|
| `overview.md` | 2026-09-14 | ✅ Fresh | **Docs sweep (post-3.2.0)**: sidecar 15 → **16** leaves (`result`); include table re-verified by compiling all 35 modules against exactly their listed sets — `lie` gains `mat3` (fails without it), `linalg_precision` gains `linalg_ext` (proven by a probe that CALLS `eigen_qr`; an empty main tolerates the undefined fn); seven `_` reaches, not six; principles: no house tolerance, `Result<T, E>` not error codes, `alloc(sizeof(T))`; Consumers: the ten live repos with pins and the reached surface, svara removed from *planned*, abaco marked plan-only. Earlier, **v3.2.0**: header line was still "v2.21.0 — 25,737 lines" (stale since 2.21.0 while every later refresh touched only the compiler line) → v3.2.0 / 26,567 lines. **v3.1.1**: compiler line → cycc **6.6.4**. Earlier, **v3.0.1**: compiler line → cycc **6.6.3**. Earlier, **v2.11.2**: compiler line → cycc **6.5.33**. Earlier, **v2.11.1**: version → 2.11.1, library line count re-measured (23,182). Earlier, **Cleanup**: ⚠ **`geo_diff` was missing from both the module map and the dependency table** since 2.10.0. Its row (`error` `geo` `quat` `vec3`) was established BY BUILDING, each dep shown load-bearing by removal; `cyrius check` is not a minimality oracle because it tolerates undefined functions in unreachable code. Earlier, **v2.11.0**: header → v2.11.0 / 23,104 lines. Earlier, **v2.10.2**: header → v2.10.2 / 22,707 lines. Earlier, **v2.10.1**: header → v2.10.1 / cycc 6.5.18 / 22,462 lines. Earlier, **v2.10.0**: header → v2.10.0, 35 math modules, 21,855 lines. Earlier, **v2.9.3**: header → v2.9.3 / cycc 6.5.16 / 21,498 lines (the exact-`orient2d` predicate). Earlier, v2.9.2: Earlier, v2.6.15: module map corrected — **BVH moved to `geo_advanced` (it was credited to `spatial`)**, `f64_le`/`f64_ge` removed from `f64_util`, CG relabelled Polak-Ribière+, error constants → `HSB_ERR_*`. |
...
| File | Filed | Status |
|---|---|---|
| `2026-09-14-cyrius-simd-dst-slot-regalloc-picker.md` | 2026-09-14 | 🔴 **OPEN upstream** — cycc register picker drops an `f64v_*` destination-slot store when a derived setter's value reads the same object through the derived getter; wrong code on 6.6.0–6.6.4 (repro exits 1 from dirs pinned to each). Filed in the cyrius repo the same day with the repro. `m3_mul_vec3` ships hoisted until a pin crosses the fix; archive then. |
