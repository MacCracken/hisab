...
> **Last refresh**: 2026-09-11 — **v2.23.0 RELEASED: the sites no fixture reached, and a defect 2.22.0
> introduced there.** 2.22.0 repaired three Householder gates *on the shape*; the roadmap carried that as
> unproven both ways. **All three were load-bearing, one also introduced a regression, and a fourth
> reflector had never been repaired.** ⭐ The new assertions fail **12** times on 2.21.0, **7** on 2.22.0,
> **6** on 2.22.1 and **0** here. ⛔ `cqr_decompose`'s `vhv` underflows below **2^-515**, and the failure
> is **invisible to both residual tests a caller would run** — `R == A` and `Q == I` make `max|QR−A|` and
> `max|QᴴQ−I|` *exactly 0* on the wrong answer. ⛔ 2.22.0's phase repair **traded 1 row right for 38
> wrong**, and a Pythagorean-triple fixture could not have seen it. ⛔ And **a gate that does not gate**:
> the CGA FNV contract is blind to magnitude — a one-token mutant makes all 1024 coefficients ±3 with the
> checksum bit-exact and all 4018 assertions passing.
>
> **Docs touched**: `CHANGELOG.md` (2.23.0), `CLAUDE.md` (status → 2.23.0, counts), `README.md`,
> `roadmap.md` (release-history row; the "repaired on the SHAPE" item **closed as PROVEN**; both CGA
> deferrals now carry a measured, patch-ready implementation plan so the next release is execution
> rather than investigation), `guides/testing.md` (**4018 → 4039**).
>
