...
> **Last refresh**: 2026-08-11 — **v2.11.1, the audit release.** A full P(-1) sweep of the 2.11.0
...
> because the guard ran *after* the sqrt; `d/dx x³ at −2` was NaN because `f64_pow` is
> `exp(n·ln(base))` and rejects negative bases. ⚠ That last is the stdlib's **stated**
...
| 📅 **Dated artifact — supersede, don't edit** | 13 | `audit/` (9 reports — the ninth is the 2026-08-11 v2.11.0 sweep), `benchmarks-rust-v-cyrius.md` (v2.2.0), `development/archive/cyrius-linalg-proposal.md` (shipped), plus the two `2026-08-04-incircle-repro*.tcyr` fixtures under `issues/`. |
| 🐞 **Open filings** | 3 | `development/issues/*` — two upstream-toolchain (`cli-arg-clobbers-source`, deliberately never re-tested because the reproducer destroys a source file, recorded as unverified rather than assumed; `dead-fn-bodies-are-never-syntax-checked`, **partially fixed in 6.5.17** and re-run unchanged on 6.5.18 — `build` and `check --with-deps` reject an unparseable file, `lint` and `vet` still exit 0) and one hisab's own (`epa-certificate`, **re-diagnosed in 2.9.3**: its central claim was false and both candidate repairs are measured and rejected; what is left is the sphere-family non-convergence, independent of the certificate). `distlib`'s self-check was **fixed upstream in 6.5.17** and archived; `for-empty-clauses` was archived on discovering upstream had closed it **WON'T-FIX by design**. ⚠ **23 archived filings sit beside the three**, and the 2026-08-11 audit adds a backlog that is tracked in `roadmap.md`'s Open items rather than as 52 separate files — 28 of those findings are reproduced but **unverified**, and nothing may be filed from them until each is re-run. |
