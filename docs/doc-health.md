...
> **Last refresh**: 2026-09-11 — **v2.21.0 RELEASED: the CGA null basis, and a pre-tag audit that
> rewrote its own headline.** 2.20.0 proved no ARITHMETIC fix existed, so this changed the BASIS: `q`
> lives in one coefficient, so the norm recovers it by subtraction instead of from a cancellation.
> Median `|P·P| / q` goes **1.0 → 4.48e-17 at 2^-30** and **1.0 → 4.33e-17 at 2^+30** — a relative
> error of 1.0 is TOTAL LOSS — and the floor is FLAT rather than a U; distance recovery **46x/49x
> wrong → 1.76e-15 / 1.93e-15**. ⛔ **"Exactly null" was the headline until the audit priced it**: it
> needs 2.18.0's Neumaier compensation dropped, which makes the norm repeat the constructor's own
> rounding — and then `1 + a·e1 + a·n0 + (a/2)·ninf` returns a scalar part of **0 where the answer is
> 1**, for 6 of 11 magnitudes. **An exact zero obtained by making the same rounding error twice is an
> artifact, not accuracy.** Reverted, and a witness written because **no assertion in 3989 could tell
> the two trees apart**. ⛔ **The first CGA call cost 2.7 ms and every benchmark was blind to it**
> because they all warm up — repaired to 0.72 ms. ⚠ **2.22x on `point*point`, up to 10.93x SLOWER at
> k = 32.** ⛔ Building it found a **cycc 6.6.2 wrong-code bug** — `continue` at two nesting levels
> binds to the wrong loop — filed upstream, and **my first filing described its own symptom wrongly**
> until the loop was instrumented. ⭐ The derivation gate came BEFORE the code, and the 2.20.0
> acceptance pins inverted exactly as designed.
>
> **Docs repaired in this pass, all of them found by the same audit**: `architecture/math.md` — the
> canonical CGA reference — still documented the ep/em basis in full, metric, blade table and product
> rule; `geo_advanced.cyr`'s module header still named `[4]=ep, [5]=em`; four constructor headlines
> and the pseudoscalar note still carried ep/em formulas; `README.md` had **three wrong filing counts
> and named a README that does not exist**; and `guides/testing.md`'s per-suite rows summed to
> **3574** against a published total of **3989**. ⭐ That last one now has a CI gate asserting each
> row against the live suite AND that the rows sum to the total, verified fail-closed three ways.
>
