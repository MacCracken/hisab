...
> **Last refresh**: 2026-09-11 — **v2.22.1 RELEASED: the subnormal residue 2.22.0 filed, and why one
> repair was not enough.** Classified against a 200-digit oracle over 3 fixtures × 52 subnormal scales:
> CORRECT **23 → 151**, LOUD **62 → 0**, SILENT **71 → 5**, zero regressions, zero LOUD → SILENT.
> ⛔ **Half one**: the balance scale came from `max|A|` alone, so a matrix spanning >1022 binades had
> its small block subnormal after the divide — and **a power-of-two divide is exact only while the
> quotient stays normal**. 18 units ÷ 8 rounds to 2, 2×8 = 16: **11% destroyed before the
> decomposition began**. ⛔ **Half two, and half one alone is worse than nothing**: it converts **30
> loud rows into silent wrong answers**. `vtv` underflows at the *bottom of the normal range*, so
> normalising `v` makes it 1 by construction. ⛔ **The filing's own conclusion was false** — "no global
> scalar can work" against f64's 2045 binades vs the ~1076 needed — and a second draft's `sc > 1` guard
> did nothing for the inputs it was written for. Both corrections are inline in the archived issue
> rather than deleted.
>
> ⛔ **A PROCESS ERROR CAUGHT BY THE USER, AND IT IS THE REASON `issues/` NOW HAS A LEDGER.** 2.22.0's
> residues were written into `roadmap.md`'s "Decisions owed" and described as *filed*, when `CLAUDE.md`
> says defects are tracked in `issues/` and discharged as a precondition of the release they gate —
> and `issues/` held nothing but `archived/`. **A roadmap row is not a filing.** The defect was filed,
> repaired in 2.22.1, and archived with its corrections; the roadmap row is reduced to the decision it
> needed and closed alongside.
>
> **Docs touched**: `CHANGELOG.md` (2.22.1 section), `CLAUDE.md` (status → 2.22.1, counts re-measured),
> `README.md`, `roadmap.md` (release-history row, Current header, residue row closed),
> `guides/testing.md` (**4014 → 4018**), `issues/archived/` (+1).
>
