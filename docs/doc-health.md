...
> **Last refresh**: 2026-09-11 — **v2.22.0 RELEASED: the Householder gate, and it was never a subnormal
> defect.** The roadmap carried this as "subnormal SVD" for three releases. `_lp_bidiagonalize` gated its
> Householder reflectors on an **absolute 1e-12 applied to a LENGTH**, and when the gate fired the
> reflector was skipped while the extraction still read only the bidiagonal band — so the entries it
> would have zeroed were **silently dropped** and the routine returned the factorisation of a DIFFERENT
> matrix with `rc = HSB_ERR_NONE`. ⭐ **The boundary is a block ratio of 2^-40 — a condition number of
> ~1e12, 982 binades above where the roadmap placed it**; 97.9% of the silent wrong answers are at NORMAL
> block ratios. Oracle-free (`prod(S)` = `|det A|`): **25 of 60 → 0**, smallest singular value 9.89× too
> large → exact. ⛔ `_lp_tridiagonalize` has the identical gate, so **`eigen_qr` returned the smallest
> eigenvalue with the WRONG SIGN**, and the values returned are exactly the spectrum of the matrix you
> get by dropping the skipped entry. ⛔ A **second, separable** defect froze `_lp_bidiag_qr`'s active
> block at 2×2 — and a 2×2 fixture cannot expose it, which is why the 2.20.0 acceptance family missed it.
> ⛔ **The roadmap's own proposed repair was at the wrong pipeline stage** and the 2.19.0 refutation that
> blocked this for three releases is TRUE. ⚠ **Seventh consecutive release where the row was wrong about
> scope**: 5 sites, 3 functions, 3 public entry points, none named by the row.
>
> **Docs touched in this pass**: `CHANGELOG.md` (2.22.0 section), `CLAUDE.md` (status → 2.22.0, counts
> re-measured, arc paragraph), `README.md` (version/tests/lines + Security lead), `roadmap.md` (train row,
> release-history row, the subnormal-SVD backlog item CLOSED with its residue filed as two new open
> items), `guides/testing.md` (**3991 → 4014**; the CI gate added in 2.21.0 caught the counts, which is
> the first time that gate has done work).
>
