...
cyrius test tests/modules.tcyr      # 1668 per-module tests
...
| `modules.tcyr` | 1668 | Per-module — geo, calc, num, complex, Lie, diffgeo, symbolic, autodiff, interval, tensor, einsum, mat3, noise, color, arena, spatial (BVH, kd-tree, spatial hash), differentiable geometry (`geo_diff` — ray/surface jets, added 2.10.0) and collision (convex hull, triangulation, Delaunay, half-edge, GJK/EPA, MPR, time-of-impact, island detection, sequential-impulse) |
...
| Geometry | ray_sphere, ray_aabb, ray_triangle, ray_capsule |
| Differentiable geometry | jet_sphere, jet_plane, jet_triangle — each paired with its primal row above so the jet/primal ratio is readable straight off the CSV |
