...
fn main() {
    ...
    match ganit::geo::ray_sphere(&ray, &sphere) {
    ...
    let area = ganit::calc::integral_simpson(|x| x * x, 0.0, 1.0, 100);
    println!("∫₀¹ x² dx ≈ {area:.6}");

    // Root finding (sqrt of 2)
    let sqrt2 = ganit::num::newton_raphson(|x| x * x - 2.0, |x| 2.0 * x, 1.0, 1e-10, 100)
